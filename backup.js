...
const os = require("os"); // 🔥 Important for Vercel temp folder
...
module.exports = async function doBackup() {
  ...
  // 🔥 USE VERCEL SAFE TEMP DIR
  const tmpRoot = os.tmpdir(); // /tmp (Allowed on Vercel)
  ...
  await new Promise((resolve, reject) => {
    ...
    archive.pipe(output);
    csvPaths.forEach((p) => archive.file(p, { name: path.basename(p) }));
    archive.finalize();
  });
